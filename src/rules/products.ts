...
function add(logger: Logger, actions: any[], doc: Product): ActionsTaken {
...
function calculate(logger: Logger, actions: any[], doc: Product): ActionsTaken {
	const results: ActionsTaken = {
		original: doc,
		modified: { identifier: doc.identifier, values: {} },
		modifications: 0
...
function clear(logger: Logger, actions: any[], doc: Product): ActionsTaken {
	const results: ActionsTaken = {
		original: doc,
		modified: { identifier: doc.identifier, values: {} },
		modifications: 0
...
function concatenate(logger: Logger, actions: any[], doc: Product): ActionsTaken {
	const results: ActionsTaken = {
		original: doc,
		modified: { identifier: doc.identifier, values: {} },
		modifications: 0
...
function copy(logger: Logger, actions: any[], doc: Product): ActionsTaken {
...
*/
	...
	for (const action of actions) {
		if (action.from_field !== undefined &&
			...
			let fromValue: any = '';
			switch (actions.fromValue_field) {
				case 'family':
					fromValue = doc[action.from_field];
					break;
				case 'categories':
					fromValue = doc[action.from_field];
					break;
				...
				default:
					fromValues = true;
					if (doc.values[action.from_field] !== undefined) {
						const map: Map<string, any> = new Map();
						for (const valuesObject of doc.values[action.from_field]) {
					...
					fromValue = map.get(key) !== undefined ? may.get(key) : '';
			}
			...
			switch (actions.to_field) {
				case 'family':
					currentValue = doc[action.to_field];
					break;
				case 'categories':
					currentValue = doc[action.to_field];
					break;
				...
				default:
					currentValues = true;
					if (doc.values[action.to_field] !== undefined) {
						const map: Map<string, any> = new Map();
						for (const valuesObject of doc.values[action.to_field]) {
							const locale: string = valuesObject.locale ? valuesObject.locale : '';
							const scope: string = valuesObject.scope ? valuesObject.scope : '';
							const data: any = valuesObject.data ? valuesObject.data : '';
							map.set(`${locale}|${scope}`, data);
					...
					currentValue = map.get(key) !== undefined ? may.get(key) : '';
			}
			if (fromValues && currentValues) {
				...
				if (fromValue !== currentValue) {
					...
					results.modified.values[actions.to_field] = [];
					for (const existingValuesObject of doc.values[action.to_field]) {
						...
						if (locale !== currentLocale &&
							scope !== currentScope) {
							results.modified.values[actions.to_field].push(existingValuesObject)		
						} else {
							results.modified.values[actions.to_field].push(valuesObject);		
						}
					}
				} 
			} else
...
function remove(logger: Logger, actions: any[], doc: Product): ActionsTaken {
	const results: ActionsTaken = {
		original: doc,
		modified: { identifier: doc.identifier, values: {} },
		modifications: 0
...
function set(logger: Logger, actions: any[], doc: Product): ActionsTaken {
	const results: ActionsTaken = {
		original: doc,
		modified: { identifier: doc.identifier, values: {} },
		modifications: 0
...
function between(logger: Logger, value: any, value: any): number {
...
function contains(logger: Logger, value: any, value: any): number {
	const methodName: string = 'contains';
	const fieldType: string = comparisonType(logger, field);
	const valueType: string = comparisonType(logger, value);
	/*
...
function empty(logger: Logger, value: any, value: any): number {
	/*
	In PHP:
...
function equalTo(logger: Logger, value: any, value: any): number {
	const methodName: string = 'contains';
...
function greaterThan(logger: Logger, value: any, value: any): number {
	
	return 0;
}

function inChildren(logger: Logger, value: any, value: any): number {
	
	return 0;
}

function inOrUnclassified(logger: Logger, value: any, value: any): number {
	
	return 0;
}

function lessThan(logger: Logger, value: any, value: any): number {
	
	return 0;
}

function startsWith(logger: Logger, value: any, value: any): number {
	
	return 0;
}

function unclassified(logger: Logger, value: any, value: any): number {
...
export async function evaluate(logger: Logger, rules: any[], doc: any): Promise<any> {
	...
	const results: any = {
		original: doc,
		modified: { identifier: doc.identifier },
		modifications: 0
	};
			...
			for (const condition of rule.conditions) {
				...
				switch (condition.field) {
					...
					default:
						...
						field = map.get(key) !== undefined ? may.get(key) : '';
				}
...
//  results = await evaluate(logger, null, null);
/*
	try {
	    const config = yaml.load(fs.readFileSync('../akeneo-pim-4.0-customizations/StrikeTru/Scripts/rules/categorize_tmpskus_simples.yml', 'utf8'));
...
/*
	  const aBoolean: any = true;
	  const aDate: any = new Date();
	  const anArray: any = ['one','two','three'];
	  const anObject: any = { one: 1, two: 'two', three: anArray };
	  const aNull: any = null;
	  const aNumber: any = 1.732;
	  const aString: any = '1.732';

	  console.log(aBoolean.toString());
...
/*
	...
	if (productsMap.size === 0) {
		// await akeneo.exportProducts();
		await mapper.load(logger, `${exportPath}${path.sep}${akeneo.filenameProducts}`, productsMap, 'identifier');
		logger.info({ moduleName, methodName, count: productsMap.size }, `Products loaded.`);
	}
...
// /Users/don/work/node-akeneo/data/products.json
	const comparisonTypesMap: Map<string, number> = new Map();
	const filename: string = '/Users/don/work/node-akeneo/data/products.json';
/*
	function liner(): Promise<any> {
		return new Promise((resolve, reject) => {
		    const stream: Readable = fs.createReadStream(filename);

			const lines: any = readline.createInterface({
			  input: stream,
			  crlfDelay: Infinity
			});
